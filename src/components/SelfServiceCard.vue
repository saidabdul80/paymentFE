<template>
  <v-card
    class="tw-p-8 border-1 tw-flex tw-mb-4"
  ...
  >
    <div>
      <span
        :class="card.selected ? `tw-bg-[${this.$constants.light}]`: card.iconBgClass"
    ...
    <div class="tw-ml-4">
      <span
        class="tw-font-bold leading-none tw-mb-4"
        :class="`${$constants.text_size.s4} tw-text-[${$constants.primary}]`"
      >
        {{ card.title }}
      </span>
      <p
        :class="`${$constants.text_size.s2} tw-text-[${$constants.dark}]`"
        class="tw-font-thin tw-m-1"
      >
        {{ card.content }}
      </p>
    </div>
