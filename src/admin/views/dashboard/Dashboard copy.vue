    ...
    <div class="tw-px-4">
        ...
        <div class="md:tw-p-5">
            ...
            <div v-else>
              ...
              <!--   <div
                    class="tw-grid md:tw-grid-cols-2 tw-grid-cols-1 lg:tw-grid-cols-3 xl:tw-grid-cols-5 tw-gap-4 tw-mb-4">
                    <statistic-card v-for="card in receivedCards" :key="card.label" :icon="card.icon" icon-color="#444"
                        :value="card.value" :label="card.label" :percentage="card.percentage"
      ...
      <!--   <v-divider class="tw-border-gray-400"></v-divider>
        <div class="md:tw-p-5">
            ...
            <div v-else class="tw-grid md:tw-grid-cols-2 tw-grid-cols-1 lg:tw-grid-cols-3 xl:tw-grid-cols-5 tw-gap-4">
                <statistic-card v-for="card in sentCards" :key="card.label" :icon="card.icon" icon-color="#444"
                    :value="card.value" :label="card.label" :percentage="card.percentage"
                    :percentage-icon="card.percentageIcon" :button-color="card.buttonColor" :card="card" />
            </div>
        </div> -->
    ...
    methods: {
        ...
        async fetchDashboards() {
            ...
            if (this.chart) {
                ...
                this.chart.data.datasets[0].data = totalReceived;
                this.chart.data.datasets[1].data = totalTransactions;
                this.chart.data.datasets[2].data = completedTransactions;
                this.chart.data.datasets[3].data = failedTransactions;
                this.chart.data.datasets[4].data = averageTransactionValue;
                this.chart.update();
            } else {
